...
- **Editar imagen**: el dropdown de 3 puntos de cada card (y el menú del detalle) incluye **Editar** (solo dueño/admin) — nombre, categoría y descripción vía `PUT /api/images/{id}`; el feed, el perfil y el detalle se actualizan al instante y el realtime propaga `ImageUpdated` a los demás clientes.
- **Hover de cards**: al pasar el mouse la imagen hace zoom out/in y el fondo muestra el **color dominante del thumbnail** (extraído con el mismo `extractDominantColor`, cacheado) — solo durante el hover.
